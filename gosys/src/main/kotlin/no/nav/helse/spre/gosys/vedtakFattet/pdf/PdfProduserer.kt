...
class PdfProduserer(
    ...
    fun lagPdf(
        ...
        val pdf =
            if (meldingOmVedtakJson["yrkesaktivitetstype"].asText() == "SELVSTENDIG") {
                runBlocking {
                    pdfClient.hentSNVedtakPdf(
                        lagSNPdfPayload(
                            ...
                            forsikringsvurdering =
                                meldingOmVedtakJson["forsikringsvurderingId"]
    ...
    private fun lagPdfPayload(
        ...
        VedtakPdfPayload(
            ...
            skjønnsfastsettingtype =
                if (meldingOmVedtakJson["sykepengegrunnlagsfakta"]["skjønnsfastsettingårsak"]?.let { enumValueOf<Skjønnsfastsettingårsak>(it.asText()) } == Skjønnsfastsettingårsak.ANDRE_AVSNITT) {
            ...
            skjønnsfastsettingårsak =
                meldingOmVedtakJson["sykepengegrunnlagsfakta"]["skjønnsfastsettingårsak"]?.let {
    ...
    private fun lagSNPdfPayload(
        ...
        SNVedtakPdfPayload(
            ...
            begrunnelser = meldingOmVedtakJson.toBegrunnelser(),
            vedtakFattetTidspunkt = meldingOmVedtakJson["vedtakFattetTidspunkt"].asLocalDateTime(),
            individuellForsikringNavn = forsikringsvurdering?.individuellForsikringNavn,
    ...
    private fun Utbetaling.toLinjer(
        navn: String,
    ): List<VedtakPdfPayload.Linje> =
        (
            arbeidsgiverOppdrag.utbetalingslinjer.map { it.toLinje(navn = "Arbeidsgiver", mottakerType = VedtakPdfPayload.MottakerType.Arbeidsgiver) } +
    ...
    ): List<VedtakPdfPayload.IkkeUtbetalteDager> =
        utbetaling
            .utbetalingsdager
    ...
    private fun mapBegrunnelseTilIkkeUtbetalteDagBegrunnelse(begrunnelse: String): String =
        when (begrunnelse) {
            "SykepengedagerOppbrukt" -> {
                "Dager etter maksdato"
            }

            "SykepengedagerOppbruktOver67" -> {
                "Dager etter maksdato - Personen er mellom 67 og 70 år, jf § 8-51"
            }

            "MinimumInntekt" -> {
                "Krav til minste sykepengegrunnlag er ikke oppfylt"
            }

            "MinimumInntektOver67" -> {
                "Krav til minste sykepengegrunnlag er ikke oppfylt - Personen er mellom 67 og 70 år, jf § 8-51"
            }

            "EgenmeldingUtenforArbeidsgiverperiode" -> {
                "Egenmelding etter arbeidsgiverperioden"
            }

            "MinimumSykdomsgrad" -> {
                "Sykdomsgrad under 20 %"
            }

            "ManglerOpptjening" -> {
                "Krav til 4 ukers opptjening er ikke oppfylt"
            }

            "ManglerMedlemskap" -> {
                "Krav til medlemskap er ikke oppfylt"
            }
            ...
            "AndreYtelserDagpenger" -> {
                "Personen mottar Dagpenger"
            }

            "AndreYtelserForeldrepenger" -> {
                "Personen mottar Foreldrepenger"
            }

            "AndreYtelserOmsorgspenger" -> {
                "Personen mottar Omsorgspenger"
            }

            "AndreYtelserOpplaringspenger" -> {
                "Personen mottar Opplæringspenger"
            }

            "AndreYtelserPleiepenger" -> {
                "Personen mottar Pleiepenger"
            }

            "AndreYtelserSvangerskapspenger" -> {
                "Personen mottar Svangerskapspenger"
            }

            "MeldingTilNavDagUtenforVentetid" -> {
                "Melding til Nav dag utenfor ventetid"
            }

            "AvslåttMeldingTilNavDag" -> {
                "Melding til Nav dag avslått av saksbehandler"
            }
