...
internal class SpForsikringClientTest {
    ...
    @Test
    fun `mapper individuell forsikring som er lagt til grunn og kollektiv forsikring`() =
        runBlocking {
            ...
            assertEquals("Valgt individuell forsikring", forsikringsvurdering?.individuellForsikringNavn)
    ...
    @Test
    fun `individuell forsikringsnavn blir null når ingen er lagt til grunn`() =
        runBlocking {
            val id = UUID.randomUUID()
            stubForsikringsvurdering(
                id,
            ...
            assertNull(forsikringsvurdering?.individuellForsikringNavn)
            assertEquals("Kollektiv forsikring", forsikringsvurdering?.kollektivForsikringNavn)
        }

    @Test
    fun `dekning blir null når samletDekning er null`() =
        runBlocking {
            val id = UUID.randomUUID()
            stubForsikringsvurdering(
                id,
    ...
    @Test
    fun `kollektiv forsikringsnavn blir null når kollektivForsikring er null`() =
        runBlocking {
            val id = UUID.randomUUID()
            stubForsikringsvurdering(
                id,
    ...
    @Test
    fun `kollektiv forsikringsnavn blir null når feltet mangler`() =
        runBlocking {
            val id = UUID.randomUUID()
            stubForsikringsvurdering(
                id,
    ...
    private fun stubForsikringsvurdering(
        id: UUID,
        status: Int,
        body: String,
    ) {
        WireMock.stubFor(
            WireMock
                .get(WireMock.urlEqualTo("/forsikringsvurderinger/$id"))
                .willReturn(
                    WireMock
                        .aResponse()
                        .withStatus(status)
